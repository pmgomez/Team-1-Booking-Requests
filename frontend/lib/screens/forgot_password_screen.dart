...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  Future<void> _submitForgotPassword() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _errorMessage = data['message'] ?? data['error'] ?? 'Failed to send reset code';
        });
      }
    } catch (e) {
      setState(() {
  ...
  Future<void> _verifyAndResetPassword() async {
    ...
    try {
      ...
      final verifyResponse = await ApiConfig.get('/api/auth/verify-reset-code/${_codeController.text.trim()}');
      ...
      if (resetResponse.statusCode == 200) {
        ...
        setState(() {
          _errorMessage = data['message'] ?? data['error'] ?? 'Failed to reset password';
        });
      }
    } catch (e) {
      setState(() {
